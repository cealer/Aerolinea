...
namespace Aerolinea_AccesoDatos
{
    public class Operaciones : BDComun
    {
        ...
        public DataTable SelectAllCondicional<unknowtype>(unknowtype Entities, string Buscar, string where, string dato)
        {
            StringBuilder innerjoin = new StringBuilder();
            string Table = getTable(Entities);
            var atributos = Entities.GetType().GetMethods();
            ...
            string consulta = "Select " + Buscar + " From " + Table + innerjoin.ToString() + " where " + where + " like @" + where + " + '%'";
            SqlDataAdapter adapter = new SqlDataAdapter(consulta, cn);
            adapter.SelectCommand.Parameters.AddWithValue("@" + where, dato);
            DataSet ds = new DataSet();
            adapter.Fill(ds, Table);
            return ds.Tables[0];
        }
